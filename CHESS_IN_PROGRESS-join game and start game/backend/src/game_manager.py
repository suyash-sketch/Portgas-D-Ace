...
class GameManager:
    ...
    def add_creator(self, user, game_token):
        """Add a user who is creating a new game."""
        self.users[user['websocket']] = user
        print(f"User {user['id']} added as game creator with token {game_token}")
        
        # Store the game with its token
        game = {
            'id': game_token,
            'creator': user,
            'joiner': None,
            'moves': [],
            'status': 'waiting'  # waiting for joiner
        }
        
        self.games[game_token] = game
        return True

    def add_joiner(self, user, game_token):
        """Add a user who is joining an existing game."""
        game = self.games.get(game_token)
        if not game or game['status'] != 'waiting' or game['joiner']:
            print(f"Cannot join game {game_token}: game not found or not in waiting status")
            return False
        
        self.users[user['websocket']] = user
        print(f"User {user['id']} added as game joiner for token {game_token}")
        
        # Add joiner to the game
        game['joiner'] = user
        game['status'] = 'active'
        
        # Randomly decide who plays as white/black
        if random.random() < 0.5:
            white, black = game['creator'], user
        else:
            white, black = user, game['creator']
